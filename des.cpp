...
ulong des_encrypt_ecb(uchar *src, ulong srclen, uchar *dst, ulong64 key)
{
	...
	//ulong = 32 бит
	//ulong64 = 64 бит
	ulong64 block = {0,0};	// 64 битный блок текста (8 символов)
	ulong64 keys[16];		
	des_create_keys(key, keys);
	for (ulong i = 0; i <= srclen; i+=8)
	{
		GET_ULONG_BE(block.l, src, i);
		GET_ULONG_BE(block.h, src, i+4);
		block = des_encrypt_block(block, keys);
		PUT_ULONG_BE(block.l, dst, i);
		PUT_ULONG_BE(block.h, dst, i+4);
	}
	return 0;
}
ulong des_decrypt_ecb(uchar *src, ulong srclen, uchar *dst, ulong64 key) 
{
	...
	for (ulong i = 0; i <= srclen; i += 8)
	{
		GET_ULONG_BE(block.l, src, i);		// Первые 4 байт
		GET_ULONG_BE(block.h, src, i + 4);	// Вторые 4 байт
		block = des_decrypt_block(block, keys);
		PUT_ULONG_BE(block.l, dst, i);		// Первые 4 байт выходного потока байт
		PUT_ULONG_BE(block.h, dst, i + 4);	// Вторые 4 байт выходного потока байт
	}
	return 0;
}
// CBC
ulong des_encrypt_cbc(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	for (ulong i = 0; i <= srclen; i += 8)
	{
		...
		GET_ULONG_BE(block.h, src, i + 4);
		if (i == 0) {
			block.l = block.l ^ iv.l;
			block.h = block.h ^ iv.h;
		}
		else
		{
			block.l = block.l ^ prevblock.l;
			block.h = block.h ^ prevblock.h;
		}
		block = des_encrypt_block(block, keys);
		PUT_ULONG_BE(block.l, dst, i);
		PUT_ULONG_BE(block.h, dst, i + 4);
...
ulong des_decrypt_cbc(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv) 
{
	ulong64 block = { 0,0 };
	ulong64 prevblock = { 0,0 };
	ulong64 afterdes = { 0,0 };
	ulong64 beforedes = { 0,0 };
	ulong64 keys[16];
	des_create_keys(key, keys);
...
{
	ulong block = 0;
	// Сдвигаемый блок-регистр, t=32, остаток = 32
	ulong64 Tblock = { 0,0 };
	ulong64 desTblock = { 0,0 };
	ulong64 keys[16];
	des_create_keys(key, keys);
	Tblock = iv;
	for (ulong i = 0; i <= srclen; i += 4)// Берем по 32 бита (4 символа)
	{
		desTblock = des_encrypt_block(Tblock, keys);
		GET_ULONG_BE(block, src, i);// block = 4 символа входной строки
		block = desTblock.h^block;	// t-->XOR<--Исходный текст
		PUT_ULONG_BE(block, dst, i);
		Tblock.h = Tblock.l;// Сдвиг регистра влево
		Tblock.l = block;	// t = полученный шифротекст
...
ulong des_decrypt_cfb(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	for (ulong i = 0; i <= srclen; i += 4)// Берем по 32 бита (4 символа)
	{
		...
		GET_ULONG_BE(block, src, i);// block = 4 символа входной строки
		prevblock = block;
		block = desTblock.h^block;	// t-->XOR<--Шифротекст

		PUT_ULONG_BE(block, dst, i);
		Tblock.h = Tblock.l;	// Сдвиг регистра влево
		Tblock.l = prevblock;	// t = исходный блок шифротекста
...
ulong des_encrypt_ofb(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	for (ulong i = 0; i <= srclen; i += 4)// Берем по 32 бита (4 символа)
	{
		...
		GET_ULONG_BE(block, src, i);// block = 4 символа входной строки
		block = desTblock.h^block;	// t-->XOR<--Исходный текст
		PUT_ULONG_BE(block, dst, i);// dst[i]=block
		Tblock.h = Tblock.l;		// Сдвиг регистра влево
		Tblock.l = desTblock.h;		// t = DES(Tblock)
...
{
	des_encrypt_ofb(src, srclen, dst, key, iv);
	/*ulong block = 0;
	// Сдвигаемый блок-регистр, t=32, остаток = 32
	ulong64 Tblock = { 0,0 };
	ulong64 desTblock = { 0,0 };
